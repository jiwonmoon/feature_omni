...
namespace F_test
{
	...
	public:
		...
		void compare2img(const vector<Mat> img_set, const int idx1, const int idx2, const Mat mask, const Mat cube_mask, int typeOfFeature, int match_tpye, std::vector<cv::Mat> LUT);
		void translate_test(const vector<Mat> img_set, const vector<Mat> img_P_vec, const vector<vector<Point2f>> keypoints_F, const vector<vector<Point2f>> keypoints_P, const Mat mask, const Mat cube_mask);
		Frame* frame_1;
		Frame *frame_2;
