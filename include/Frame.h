...
namespace F_test
{
	...
	public:
		...
		Frame(const Mat img, const Mat mask, const Mat cube_mask, ORBextractor* ORBextract, std::vector<cv::Mat> LUT);


		void Extract_ORB(Mat img, Mat mask);///
		void Extract_BRISK(Mat img, Mat mask);///
		void Extract_AKAZE(Mat img, Mat mask);///
		void Extract_ORB_EX(Mat img, Mat mask, const Mat cube_mask, std::vector<cv::Mat> LUT);
