...
namespace F_test
{
	...
	void Compare::compare2img(const vector<Mat> img_set, const int idx1, const int idx2, const Mat mask, const Mat cube_mask, int typeOfFeature, int match_type, std::vector<cv::Mat> LUT) {
		cout << "[TwoViewTest >> compare2img (" << idx1 << " vs " << idx2 << ")]" << endl << endl;

	Mat t1 = img_set[idx1].clone();
	Mat t2 = img_set[idx2].clone();
		...
		case 2:
			...
			break;
		case 3:
			frame_1 = new Frame(img_set[idx1], mask, cube_mask, mpORBextractor, LUT);

			frame_2 = new Frame(img_set[idx2], mask, cube_mask, mpORBextractor, LUT);
		...
		double match_T = (double)(clock() - match_start);
		...
		N1_vec.push_back(frame_1->N);
		N2_vec.push_back(frame_2->N);
		avgN_vec.push_back(num_avg_Keypoint);
		HammingDistance.push_back(Avg_dist);
		avgGoodMatch_vec.push_back(goodMatches.size());
		avgRate_vec.push_back(recog_Rate);

		TIME_avgGoodMatch_vec.push_back(match_T);
