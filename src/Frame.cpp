...
namespace F_test
{
	...
	Frame::Frame(const Mat img, const Mat mask, const Mat cube_mask, ORBextractor* ORBextract, std::vector<cv::Mat> LUT)
	{
		F_img = img.clone();
		...
		// ORB extraction
		//Extract_ORB_EX_1(img, mask, cube_mask);
		Extract_ORB_EX(img, mask, cube_mask, LUT);
...
#ifdef DEBUG
		cout << "Detection T: " << duration_detect << " , per Point: " << duration_detect / mvKeys.size() << " (# of point: " << mvKeys.size() << ") " << endl;
		cout << "Description T: " << duration_compute << " , per Point: " << duration_compute / mvKeys.size() << " (# of point: " << mvKeys.size() << ") " << endl;
#endif
	}
	...
	*/
	...
	void Frame::Extract_ORB_EX(Mat img, Mat mask, const Mat cube_mask, std::vector<cv::Mat> LUT)
	{
		...
		cv::Mat LUT_x = LUT[0];
		cv::Mat LUT_y = LUT[1];
		cv::Mat LUT_x_inv = LUT[2];
		cv::Mat LUT_y_inv = LUT[3];
		// ORB extraction
		(*mpORBextractor)(img, mask, mvKeys, mDescriptors, LUT_x, LUT_y, LUT_x_inv, LUT_y_inv);
