...
namespace F_test
{
	...
	void System::TwoViewTest(const vector<Mat> img_vec, const Mat mask, std::vector<cv::Mat> LUT, const Mat cube_mask, int img_term, int delay)
	{
		int matcher_type = 0;
		...
		{
			compare_->compare2img(img_vec, 0, ni + img_term, mask, cube_mask, typeOffeature, matcher_type, LUT);
		}
	}
	...
	void System::print_RESULT()
	{
		...
		for (int i = 0; i < avgRate_vec.size(); i++)
		{
			...
			cout << N1_vec[i] << "\t" << N2_vec[i] << "\t" << avgGoodMatch_vec[i] << "\t" << HammingDistance[i] << "\t" <<  TIME_DC1_vec[i]/1000 << "\t"<<  TIME_DC2_vec[i]/1000 << "\t"<< TIME_avgGoodMatch_vec[i]/1000  << endl;
			fout << N1_vec[i] << "\t" << N2_vec[i] << "\t" << avgGoodMatch_vec[i] << "\t" << HammingDistance[i] << "\t" <<  TIME_DC1_vec[i]/1000 << "\t"<<  TIME_DC2_vec[i]/1000 << "\t"<< TIME_avgGoodMatch_vec[i]/1000  << endl;
		}
